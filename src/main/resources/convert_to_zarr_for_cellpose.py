...
Axes convention used throughout this pipeline: (Z, Y, X) or (C, Z, Y, X).
This keeps channels explicit in the OME-Zarr metadata and avoids viewers
guessing that a trailing axis of length 3 should be interpreted as RGB.
...
def _simplify_array(data):
    """Drop singleton channel dim when C=1 -> store as (Z,Y,X) not (C,Z,Y,X)."""
    if data.ndim == 4 and data.shape[0] == 1:
        return data[0, ...]
    return data


def _to_ome_zarr_layout(data):
    """Return data in ZYX or CZYX layout for writing."""
    if data.ndim == 4:
        return np.moveaxis(data, -1, 0)
    return data


def _base_chunks_for_array(data, chunk_values):
    """Map ZxYxX chunk input to the in-memory array layout used for writing."""
    if data.ndim == 4:
        cv = chunk_values + [chunk_values[-1]] * max(0, 3 - len(chunk_values))
        return (int(data.shape[0]), cv[0], cv[1], cv[2])
    cv = chunk_values + [chunk_values[-1]] * max(0, data.ndim - len(chunk_values))
    return tuple(cv[: data.ndim])
...
def _read_ims(path):
    ...
    All channels are stacked as (C, Z, Y, X); single-channel data is returned
    as (Z, Y, X).
    ...
    data = channels[0] if len(channels) == 1 else np.stack(channels, axis=0)
...
def _read_image(input_path):
    ...
    * Otherwise   → bioio (Bio-Formats, OME-TIFF, plain TIFF, …)
    ...
    Returns ``(ndarray ZYX-or-CZYX, pps)`` where ``pps`` exposes ``.X``,
    ...
    if data.ndim == 5 and data.shape[0] == 1:
        data = data[0]  # (Z, Y, X, C)
    return _to_ome_zarr_layout(data), pps
...
def _spatial_axes(data):
    """Return the indices of spatial axes for ZYX or CZYX arrays."""
    if data.ndim == 4:
        return [1, 2, 3]  # (C, Z, Y, X)
...
def _get_zarr_group(path, mode="w"):
    ...
    is_network_drive = "U:\\" in path or "U:/" in path
    ...
    if is_network_drive:
        ...
        # Zarr 3 LocalStore always writes via atomic temp-file renames, which can
        # fail on SMB shares. Use an async-wrapped fsspec filesystem instead.
        fs = AsyncFileSystemWrapper(
            fs=fsspec.filesystem("file", auto_mkdir=True),
            asynchronous=True,
        )
        store = FsspecStore(fs, path=path)
        group = zarr.open_group(store=store, mode=mode, zarr_format=2)
    else:
        try:
            # Zarr 3.x: We MUST use zarr_format=2 for OME-Zarr 0.4.
            group = zarr.open_group(path, mode=mode, zarr_format=2, synchronizer=None)
        except TypeError:
            # Zarr 2.x: zarr_format is not a valid argument
            group = zarr.open_group(path, mode=mode, synchronizer=None)

    _ensure_direct_zattrs(path)
    return group


def _build_omero_channels(channel_count):
    default_colors = [
        "FF0000",
        "00FF00",
        "0000FF",
...
def _get_store_path(zarr_obj):
    return getattr(getattr(zarr_obj, "store_path", None), "path", None)


def _ensure_direct_zattrs(store_path):
    zattrs_path = os.path.join(str(store_path), ".zattrs")
    os.makedirs(str(store_path), exist_ok=True)
    if not os.path.exists(zattrs_path):
        with open(zattrs_path, "w", encoding="utf-8") as handle:
            json.dump({}, handle, ensure_ascii=True, separators=(",", ":"))
    return zattrs_path


def _update_attrs_with_fallback(zarr_obj, updates=None, remove_keys=None):
    updates = updates or {}
    ...
    try:
        ...
        for key in remove_keys:
            if key in zarr_obj.attrs:
                del zarr_obj.attrs[key]
        return
    except (PermissionError, OSError) as exc:
        logger.warning(
            "Could not update Zarr attributes via metadata API; "
            "falling back to direct .zattrs write: %s",
            exc,
        )

    store_path = _get_store_path(zarr_obj)
    if store_path is None:
        logger.warning("Skipping attribute update fallback: dataset path unavailable.")
        return

    zattrs_path = _ensure_direct_zattrs(store_path)
    attrs = {}
    try:
        with open(zattrs_path, "r", encoding="utf-8") as handle:
            attrs = json.load(handle)
    except Exception as exc:
        logger.warning("Could not read existing .zattrs at %s: %s", zattrs_path, exc)
        attrs = {}
    ...
    with open(zattrs_path, "w", encoding="utf-8") as handle:
        json.dump(attrs, handle, ensure_ascii=True, separators=(",", ":"))


def _write_metadata(zstore, pixel_sizes, axes, datasets_meta, input_name, channel_count=1):
        ...
        "multiscales": [
            {
                ...
                "metadata": {
                    "description": (
                        ...
                        "Axes convention: ZYX or CZYX."
                    )
                },
            }
        ],
        "physical_pixel_sizes_um": pixel_sizes,
    }
    if int(channel_count) > 1:
        attrs["omero"] = {
            "version": "0.4",
            "channels": _build_omero_channels(channel_count),
        }
        remove_keys = []
    else:
        remove_keys = ["omero"]

    _update_attrs_with_fallback(zstore, updates=attrs, remove_keys=remove_keys)
...
def _annotate_array_dimensions(zarr_array, axes):
    ...
    _update_attrs_with_fallback(zarr_array, updates={"_ARRAY_DIMENSIONS": axis_names})
...
def _write_ome_zarr(output_path, data, base_chunks, pixel_sizes, n_levels, input_name):
    ...
    ndim = data.ndim
    n_ch = data.shape[0] if ndim == 4 else 1
    ...
    if not (ndim == 3 or ndim == 4):
        ...
        storage_chunks = (
            (chunks0[0], chunks0[1], 1024, 1024)
            if ndim == 4
            else (chunks0[0], 1024, 1024)
        )
        print(
    ...
    chunk_z = chunks0[1] if ndim == 4 else chunks0[0]
    z_size = data.shape[1] if ndim == 4 else data.shape[0]
    y_size = data.shape[-2]
    x_size = data.shape[-1]
    ...
    def _write_slab(z_start):
        ...
        for ys in range(0, y_size, tile_y):
            ye = min(ys + tile_y, y_size)
            for xs in range(0, x_size, tile_x):
                xe = min(xs + tile_x, x_size)

                slab_tile = (
                    data[z_start:z_end, ys:ye, xs:xe]
                    if ndim == 3
                    else data[:, z_start:z_end, ys:ye, xs:xe]
                )
    ...
    def _append_ds_meta(path_str, sf):
        ...
        if ndim == 3:
            ...
            scale_values = [1.0, pz * sf, py * sf, px * sf]
    ...
    _write_metadata(store, pixel_sizes, axes, datasets_meta, input_name, channel_count=n_ch)
    ...
    for level in range(1, n_levels):
        ...
        src_Z = prev_shape[1] if n_ch > 1 else prev_shape[0]
        chunk_z = chunks_l[1] if n_ch > 1 else chunks_l[0]
        ...
        def _write_pyramid_tile(out_z_start, out_y_start, out_x_start):
            ...
            src_y_end = min(src_y_start + chunks_l[-2] * 32, prev_shape[-2])
            src_x_start = out_x_start * 2
            src_x_end = min(src_x_start + chunks_l[-1] * 32, prev_shape[-1])
            ...
            # Output bounds
            out_z_end = min(out_z_start + chunk_z, out_shape[1] if n_ch > 1 else out_shape[0])
            out_y_end = min(out_y_start + chunks_l[-2] * 16, out_shape[-2])
            out_x_end = min(out_x_start + chunks_l[-1] * 16, out_shape[-1])
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            for ozs in range(0, out_shape[1] if n_ch > 1 else out_shape[0], chunk_z):
                for oys in range(0, out_shape[-2], chunks_l[-2] * 16):
                    for oxs in range(0, out_shape[-1], chunks_l[-1] * 16):
...
def _write_ome_zarr_from_ims(
    ...
    volume. Multichannel data is written as (Z, C, Y, X).
    ...
    with h5py.File(ims_path, "r") as f:
        ...
        def _full_shape(level_shape):
            return level_shape if n_ch == 1 else (n_ch, level_shape[0], level_shape[1], level_shape[2])
        ...
        spatial_axes = [1, 2, 3] if n_ch > 1 else [0, 1, 2]

        chunks0 = _chunks_for_level(base_chunks, 0, full_shape)
        if n_ch > 1 and len(chunks0) == 3:
            chunks0 = (n_ch, chunks0[0], chunks0[1], chunks0[2])
    ...
    def _get_scale(level_shape):
        ...
        if n_ch > 1:
            scale = [1.0, scale[0], scale[1], scale[2]]
        return scale
    ...
    for entry in level_entries[1:]:
        ...
        if n_ch > 1 and len(l_chunks) == 3:
            l_chunks = (n_ch, l_chunks[0], l_chunks[1], l_chunks[2])

        if "U:\\" in output_path or "U:/" in output_path:
            l_storage = (
                (l_chunks[0], l_chunks[1], 1024, 1024)
                if n_ch > 1
                else (l_chunks[0], 1024, 1024)
            )
        else:
            l_storage = l_chunks
    ...
    _write_metadata(zstore, pixel_sizes, axes, datasets_meta, input_name, channel_count=n_ch)
    # ---------------------------
    ...
    if not level0_done:
        ...
        read_stride_yx = 2048
        chunk_z = chunks0[1] if n_ch > 1 else chunks0[0]
    ...
    _update_attrs_with_fallback(
        zstore,
        updates={"physical_pixel_sizes_um": pixel_sizes},
    )
...
def main():
    ...
    base_chunks = _base_chunks_for_array(data, chunk_values)
    print(f"Base chunks (level 0): {base_chunks}")
