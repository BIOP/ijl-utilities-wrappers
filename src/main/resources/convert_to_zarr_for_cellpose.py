...
Axes convention used throughout this pipeline: (Z, Y, X) or (Z, C, Y, X).
...
def _simplify_array(data):
    """Drop singleton channel dim when C=1 → store as (Z,Y,X) not (Z,C,Y,X)."""
...
def _to_ome_zarr_layout(data):
    """Return data in ZYX or ZCYX layout for writing."""
    if data.ndim == 4:
...
def _base_chunks_for_array(data, chunk_values):
    ...
    if data.ndim == 4:
        ...
        return (cv[0], int(data.shape[1]), cv[1], cv[2])
...
def _read_ims(path):
    ...
    All channels are stacked as (Z, C, Y, X); single-channel data is returned
    as (Z, Y, X).
...
def _read_image(input_path):
    ...
    * Otherwise   → bioio (Bio-Formats, OME-TIFF, plain TIFF, …)
    ...
    Returns ``(ndarray ZYX-or-ZCYX, pps)`` where ``pps`` exposes ``.X``,
...
def _spatial_axes(data):
    """Return the indices of spatial axes for ZYX or ZCYX arrays."""
    if data.ndim == 4:
        return [0, 2, 3]  # (Z, C, Y, X)
...
def _write_ome_zarr(output_path, data, base_chunks, pixel_sizes, n_levels, input_name):
    ...
    def _append_ds_meta(path_str, sf):
        ...
        if ndim == 3:
            ...
            scale_values = [pz * sf, 1.0, py * sf, px * sf]
...
def _write_ome_zarr_from_ims(
    ...
    with h5py.File(ims_path, "r") as f:
        ...
        def _full_shape(level_shape):
            return level_shape if n_ch == 1 else (level_shape[0], n_ch, level_shape[1], level_shape[2])
        ...
        if n_ch > 1 and len(chunks0) == 3:
            chunks0 = (chunks0[0], n_ch, chunks0[1], chunks0[2])
    ...
    def _get_scale(level_shape):
        ...
        if n_ch > 1:
            scale = [scale[0], 1.0, scale[1], scale[2]]
        return scale
    ...
    for entry in level_entries[1:]:
        ...
        if n_ch > 1 and len(l_chunks) == 3:
            l_chunks = (l_chunks[0], n_ch, l_chunks[1], l_chunks[2])
