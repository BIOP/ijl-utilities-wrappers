...
def _write_ome_zarr_from_ims(
    ...
    factors = (2, 2, 2) if n_ch == 1 else (2, 2, 2, 1)

    for l_idx in range(1, n_levels):
        if not _can_add_level(curr_shape, spatial_axes, base_chunks):
            break
    ...
    for level, out_shape, chunks_l, storage_chunks_l in pyramid_info:
        ...
        if not level_done:
            ...
            def _write_pyramid_tile_ims(out_z_start, out_y_start, out_x_start):
                # Tile size for pyramid building (output space)
                # We use 1024x1024 to keep memory usage per worker predictable (~256MB)
                tile_size_yx = 1024

                # Output bounds
                out_z_end = min(out_z_start + chunk_z, out_shape[0])
                out_y_end = min(out_y_start + tile_size_yx, out_shape[1])
                out_x_end = min(out_x_start + tile_size_yx, out_shape[2])

                # Input bounds (2x output)
                src_z_start = out_z_start * 2
                src_z_end = min(src_z_start + (out_z_end - out_z_start) * 2, src_Z)
                src_y_start = out_y_start * 2
                src_y_end = min(
                    src_y_start + (out_y_end - out_y_start) * 2, prev_shape[1]
                )
                src_x_start = out_x_start * 2
                src_x_end = min(
                    src_x_start + (out_x_end - out_x_start) * 2, prev_shape[2]
                )
                ...
                # Downsample tile
                ds_tile = downscale_local_mean(src_tile, factors).astype(dtype)
