...
public class SelectToROIKeepLines {
	...
	public void run(ImageProcessor ip, boolean[][] splitable, boolean filterMergable) {
		showStatus = true;
		image.setRoi(convert(ip, splitable, filterMergable));
	}
	
	/** Returns a selection created from the thresholded pixels in the
		specified image, or null if there are no thresholded pixels. */
	public static Roi run(ImagePlus imp, boolean[][] splitable, boolean filterMergable) {
		SelectToROIKeepLines tts = new SelectToROIKeepLines();
		return tts.convert(imp.getProcessor(), splitable, filterMergable);
	}
	
	/** Returns a selection created from the thresholded pixels in the
		specified image, or null if there are no thresholded pixels. */
	public Roi convert(ImageProcessor ip, boolean[][] splitable, boolean filterMergable) {
		this.ip = ip;
		min = (float)ip.getMinThreshold();
		max = (float)ip.getMaxThreshold();
		w = ip.getWidth();
		h = ip.getHeight();
		return getRoi(splitable, filterMergable);
	}
	...
	static class Outline {
		...
		public Polygon getPolygon(boolean[][] splitable, boolean filterMergable) {
			...
			for (i=first+1; i+1<last; j++) {
				...
				int y2 = y[j + 1] - y[j];
				if (x1 * y2 == x2 * y1) {
					if ((filterMergable)&&(splitable[x[j]][y[j]])) {
						// merge i + 1 into i
						last--;
						continue;	
					}
	 ...
	 */
	Roi getRoi(boolean[][] splitable, boolean filterMergable) {
		if (showStatus)
				...
				if (thisRow[x + 1]) {
					...
					if (prevRow[x + 1]) {
						...
						if (outline[x] == null) {
							...
							polygons.add(outline[x].getPolygon(splitable, filterMergable)); // PLUS
							outline[x] = outline[x + 1] = null;
						} else {
